...
const EditeUserForm = () => {
  ...
  const { id } = useParams();
  const { data: subAccountResponse, isLoading } = useGetSubAccountByIdQuery(id!);
  ...
  const initialValues: UserFormValues = {
    username: subAccount?.fullName || "",
    experience: subAccount?.lawyerExperience?.toString() || "",
    gender: subAccount?.gender || "male",
    status: subAccount?.isActive ?? true,
  ...
  const handleSubmit = async (values: UserFormValues) => {
    try {
      ...
      const updatedSubAccount = await SubAccountService.updateSubAccount(id!, payload);
      console.log("✅ Updated:", updatedSubAccount);

      window.location.href = "/lawyer/settings/myaccountsettings";

    } catch (error: any) {
      console.error("❌ Error:", error.response?.data || error.message);
      setServerError(error.response?.data?.message || "Failed to update sub account");
    }
  };
