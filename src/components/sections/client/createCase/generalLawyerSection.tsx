...
const GeneralLawyerSection = (props: Props) => {
  const { form } = props;
  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>{t("caseType.label")}</FormLabel>
          <Select onValueChange={field.onChange} value={field.value}>
            <FormControl>
              <SelectTrigger className="border-border h-12! w-full rounded-xl">
                <SelectValue placeholder={t("caseType.placeholder")} />
              </SelectTrigger>
            </FormControl>
            <SelectContent>
...
export default GeneralLawyerSection;